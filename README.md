...
本版本围绕 mask 标注流程做了几项可用性修复：

- 保存后显示真实输出预览：点击“只保存锁区”或“保存当前Mask+锁区”后，画布会切换到实际写入 COCO 的 mask 预览，避免保存结果和编辑画面不一致造成误判。
- 保存按钮语义更明确：“保存锁区”改为“只保存锁区”，“保存锁区+Mask”改为“保存当前Mask+锁区”。
- “只保存锁区”只会输出前景锁区，并扣除背景锁区；不会把当前过大的 SAM mask 一起写入 COCO。
- 鼠标越界坐标不再吸附到图片边缘：画布外的点击会被忽略，后端收到越界点也会拒绝，避免在 `0` 或最大边界处产生误点。
- 线条 prompt 和闭合锁区中的越界点会被跳过，不再自动 clamp 到边缘。
- 新增“查看编辑Mask / 查看保存结果”切换：保存后默认显示实际 COCO 输出，也可以切回编辑中的 mask；重新打开已保存目标时也能恢复保存结果预览。
- 目标下拉列表增加处理状态前缀：`未打开`、`已编辑未保存`、`已保存COCO`、`难处理`、`错误框`，批量处理时更容易发现漏保存目标。
- 点击“保存当前Mask+锁区”时，如果当前 mask 面积明显大于前景锁区，会先弹出确认提示，避免误把过大的 mask 写入 COCO。
- 后端 target metadata 增加当前 COCO 保存状态，前端可以稳定显示每个目标是否已经保存。
- 锁区后可以继续点击“迭代一次”：SAM3 每次生成新 mask 后，会把前景锁区作为强制保留区域、背景锁区作为强制扣除区域重新应用。
- 保存模式切换增加覆盖确认：如果已经保存为“只保存锁区”，再点“保存当前Mask+锁区”会提示它会覆盖当前 COCO 结果，反向切换也会提示。
- 锁区支持编辑：在“已锁定区域”列表点击“编辑”后，可以直接拖动画布上的锁区顶点；松开鼠标后自动保存并记录 `region_edit` 历史。
- 补充了相关回归测试，并在项目虚拟环境中验证 `pytest` 全量测试通过。
